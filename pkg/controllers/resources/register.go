...
func getSyncers(ctx *synccontext.RegisterContext) []BuildController {
	...
	loghelper.Infof("CSI syncer registration via controller logger: csinodes=%q csidrivers=%q csistoragecapacities=%q capacity_constructor_enabled=%t",
		ctx.Config.Sync.FromHost.CSINodes.Enabled,
		ctx.Config.Sync.FromHost.CSIDrivers.Enabled,
		ctx.Config.Sync.FromHost.CSIStorageCapacities.Enabled,
		csiStorageCapacitiesEnabled,
	)
	klog.FromContext(ctx).Info("CSI syncer registration via contextual klog",
		"csinodes", ctx.Config.Sync.FromHost.CSINodes.Enabled,
		"csidrivers", ctx.Config.Sync.FromHost.CSIDrivers.Enabled,
		"csistoragecapacities", ctx.Config.Sync.FromHost.CSIStorageCapacities.Enabled,
		"capacity_constructor_enabled", csiStorageCapacitiesEnabled,
	)
